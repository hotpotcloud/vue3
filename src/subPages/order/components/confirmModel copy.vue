  ...
  <view class="modal" v-if="show">
    ...
    <view class="main" :style="{ height: modelH }">
      ...
      <view class="main-body">
        <view
          :class="[typeName === 'num' ? 'd-flex-center' : 'd-sb']"
          class="d-se"
        >
  ...
  computed: {
    ...
    defaultCheck() {
      ...
      if (this.typeName === "long") {
        this.checked = this.planInfo?.ruleType || ruleTypeEnum.EVERY_DAYS;
        this.defaultDay = this.planInfo?.intervalDate || 1;
        this.weekVal = this.planInfo?.deliveryDetails?.split(",") || [];
        this.currentWeek = this.planInfo?.deliveryDetails?.split(",") || [];
      } else if (this.typeName === "time") {
        this.checked = this.planInfo?.timeSection || this.calcList[0].value;
      } else if (this.typeName === "num") {
        const x =
          this.planInfo?.everyNum - 1 <= 0 ? 0 : this.planInfo?.everyNum - 1;
        this.numIndex = x >= 3 ? 3 : x || 0;
        this.defaultNum = this.planInfo?.everyNum;
      }
      return;
    },
    //标题
    ...
    //空白高度
    calcEmptyH() {
      const nullH = 100 - parseInt(this.modelH.slice(0, -1)) + "%";
      return nullH;
    },
  },
    ...
    onClosePlan() {
      ...
      if (this.show) {
        ...
        if (this.typeName === "long") {
          ...
          this.checked = this.planInfo?.timeSection || timeSectionEnum.FORENOON;
        } else if (this.typeName === "num") {
          const x =
            this.planInfo?.everyNum - 1 <= 0 ? 0 : this.planInfo?.everyNum - 1;
          this.numIndex = x >= 3 ? 3 : x || 0;
