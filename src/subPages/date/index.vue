  ...
  <view class="date-out" v-else>
    ...
    >
      ...
      >
        ...
        >
          <h-calendar :val="item.deliveryInfoList" :ym="item.date" />
        </view>
        <view class="bot"
          >—
          {{ moreText }}
...
import { RuleTextEnum } from "@/utils/enum";
...
export default {
  ...
  data() {
    return {
      ...
      paramBefore: {
        // channelSkuCode: "50006276",
        // itemNo: "16558921804360330",
        // orderNo: "22062218030043506291",
        page: 1,
        size: 6,
      },
      ...
      param: {
        //uat
        // channelSkuCode: "50000015",
        // itemNo: "16595774486350808",
        // orderNo: "22080409440862702205",
        // skuCode: "50000015",
        // type: 1,
        page: 1,
        size: 10,
  ...
  onShow() {
    // const { channelSkuCode, itemNo, orderNo, skuCode, type } =
    const obj = this.$store.state.canlendar.params;
    ...
    // console.log(this.$store.state.canlendar.params, "仓库参数");
  },
  onLoad(options) {
  ...
  methods: {
    ...
    /**
     * @name 请求配送日历数据
     * @param type 查询类型(before:历史日历；next:当前日历)
     * @param p 请求参数
     */
    async postCalendar(type: string = "current", p: any = this.param) {
      ...
      //当前日历
      this.val = data;
      this.calendarList = [...this.calendarList, ...data.calendarList]; //日历数据
      // 是否有历史
