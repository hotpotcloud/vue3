  ...
  <view class="container">
    <view class="address-lists" v-if="loading">
      ...
      <view class="address-content" v-else>
        <view v-for="item in list" :key="item.addressNo" class="add-card">
          <view class="d-flex-center d-sb card_detail">
            <view @click="chooseAddress(item)" class="flex-1">
              ...
              <view class="d-flex-center add-header">
                <view
                  ...
                  >{{ item.defaulted === 1 ? "默认" : item.labelName }}</view
                >
              ...
              <view class="d-flex-center add-user">
                ...
                <view class="flex-1 h-over-1">{{ item.receiverPhone }}</view>
              </view>
            </view>
            <view>
              <uni-icons
          ...
          <view
            class="d-flex-center default_box"
            @tap.stop="selectAddress(item)"
          >
            <image
              v-if="item.defaulted === 1"
              :src="getAssetImgUrl('address-check.png')"
              mode="widthFix"
              class="check-icon"
            />
            <view class="rounder_check" v-else></view>
...
export default Vue.extend({
  ...
  methods: {
    ...
    async selectAddress(item) {
      ...
      try {
        if (item.defaulted === 1) return;
        const { code, msg } = await this.POST(
          address.defaultAddress + item.addressNo
        );
        if (code == "00000") {
          this.getList();
          uni.$u.toast(msg);
