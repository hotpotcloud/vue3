<template>
  ...
  >
    <!-- 日历顶部信息 -->
    <view class="d-flex-center d-sb topbox">
      <view class="desc">
        <!-- {{
            getAssignDateInfo(false, 0) === getAssignDateInfo(true, 0)
              ? ""
              : 
              getAssignDateInfo(false, 0) + "/"
          }}{{ getAssignDateInfo(false, 1) }} -->
        {{ getAssignDateInfo(false, 0) + "/" }}{{ getAssignDateInfo(false, 1) }}
      </view>
      <view>
        <!-- 调整日历 -->
        <slot name="btn" />
    ...
    <view class="calendar_swiper">
      ...
      <swiper
        v-else
        key="shrinkSwiper"
        :circular="weekDisable"
        class="fuckpro"
        :style="{ maxHeight: swiperHeight('close'), minHeight: '118rpx' }"
      ...
      >
        ...
        >
          ...
          />
          <!-- <view class="sw-goods">
            <swiper autoplay circular vertical="true" interval="3000">
              <swiper-item>想法子</swiper-item>
              <swiper-item>想法子2</swiper-item>
            </swiper>
          </view> -->
        </swiper-item>
      </swiper>
    ...
    <!-- 日历切换模式 -->
    <!-- @tap="swiperMode = swiperMode === 'open' ? 'close' : 'open'" -->
    <!-- <view v-if="showOff" class="calendar_toggle" @tap="changeMode">
      <view class="icon" :class="{ down: swiperMode === 'close' }"></view>
    </view> -->
    <view class="icon_box" @tap="changeMode">
      <view :class="[swiperMode === 'close' && 'week_icon']"></view>
      <view v-if="swiperMode === 'open'" class="open_iocn">
        <img
          v-if="swiperMode === 'open'"
          class="oepnimg"
          src="https://freshgo-prd-1302811166.cos.ap-chengdu.myqcloud.com/fhgo-miniprogram/commonSource/calender/Vector.png"
        />
      </view>
    </view>
  </view>
...
<script>
import { parseTime, deepClone, aroundTime } from "./js/utils.js";
...
export default {
  ...
  data() {
    return {
      ...
      watchBtn: this.mode,
      disable: true, //月视图衔接滑动
      weekDisable: true, //周视图滚动
    };
  },
  computed: {
    //周月切换
    ...
    swiperHeight() {
      return (swiperMode) => {
        ...
        // const shrinkHeight = this.cellHeight + "rpx";
  ...
  methods: {
    ...
    touchStart(e) {
      ...
      this.startX = clientX;
      this.startY = clientY;
      // console.log("e->touchStart", e, this.startX, this.startY);
    },
    touchMove(e) {
    ...
    touchEnd(e) {
      ...
      // console.log("e->touchEnd", e);
    },
    //翻页e
    changePage(e) {
      if (this.swiperMode === "open") {
        // console.log("e-滑动", e.detail.current);
        this.current = e.detail.current;
        // console.log(
        //   "this.selectedDate----->当前选中日期---翻页里面",
        //   this.selectedDate
        // );
      } else if (this.swiperMode === "close") {
        this.shrinkCurrent = e.detail.current;
      }
    },
    //月翻页
    changeMonth(e) {},
    ...
    initCalendarSwiperDates(cb) {
      ...
      // if (prev[15].month === arr[0].month && prev[15].year === arr[0].year) {
      //   console.log("calendarDate[0].month", prev[15].month, prev[15].year);
      //   this.disable = false;
      // }
      ...
      if (this.current === 0) {
        ...
        //限制月份
        // if (cur[15].month === arr[0].month && cur[15].year === arr[0].year) {
        //   console.log("calendarDate[0].month", cur[15].month, cur[15].year);
        //   this.disable = false;
        // }
        // console.log("0[cur, next, prev]", 0, this.calendarSwiperDates);
        ...
        //   this.disable = false;
        // }
        // console.log("1[prev, cur, next]", 1, this.calendarSwiperDates);
      } else if (this.current === 2) {
        this.calendarSwiperDates = [next, prev, cur];
        // if (next[15].month === arr[0].month && next[15].year === arr[0].year) {
        //   console.log("calendarDate[0].month", next[15].month, next[15].year);
        //   this.disable = false;
        // }
        // console.log("2[next, prev, cur]", 2, this.calendarSwiperDates);
      }
    ...
    generateCalendar(year, month) {
      ...
      // console.log("calendarDate--->2222初始化日期计算", calendarDate);
    ...
    initCalendarShrinkSwiperDates(swiperChangeType) {
      ...
      // console.log("cur周日历", cur);
      // const arr = aroundTime(7);
    ...
    getPrevOrNextDate(type) {
      ...
      // const scopeT = aroundTime();
      // if (
      //   scopeT[1].date === this.selectedDate ||
      //   scopeT[scopeT.length - 1].date === this.selectedDate
      // ) {
      //   this.disable = false;
      // } else {
      //   this.disable = true;
