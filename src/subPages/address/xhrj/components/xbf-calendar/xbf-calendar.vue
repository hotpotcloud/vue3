<template>
  <!-- 日历滚动插件 -->
  <view class="xbf_calendar" :class="[resize && 'resize_active']">
    <!-- 日历顶部信息 -->
    <view class="calendar_info d-sb">
      <template v-if="!showSlot">
        <view><slot name="topleft" /></view>
        <!-- <view class="h-over-1 flex-1">
          <view class="backToToday" @tap="goToDate()">回到今天</view>
          <slot name="topright"
        /></view> -->
        <view class="right-slot d-flex-center" v-if="showRight">
          <img
            class="check-icon"
      ...
      <view v-if="showSlot" class="d-flex-center d-sb">
        <view class="desc">
    ...
    <!-- 日历周数 -->
    <view class="calendar_week" :class="[resize && 'week_border']">
      <view
...
export default {
  ...
  watch: {
    // defaultSelectedDate(nv, av) {
    //   console.log("av,nv", av, nv);
    //   if (nv != av) {
    //     this.init();
    //   }
    // },
