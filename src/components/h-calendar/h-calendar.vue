      ...
      <view class="main-date">
        ...
        <view class="month d-flex-warp">
          ...
          <view
            ...
            :class="[
              i.deliveryStatus === RuletextEnum.NEW
                ? 'month-day-active'
                : i.deliveryStatus === RuletextEnum.CONFIRM
                ? 'month-day-before'
                : i.deliveryStatus === RuletextEnum.STOPPED
                ? 'month-day-stop'
                : '',
            ]"
          >
            ...
            >
              {{ RuleStatusEnum[i.deliveryStatus] }}
            </view>
          </view>
          <view
...
<script lang="ts">
import Vue from "vue";
import { getAllDate, monthDay } from "./date";
import { RuleStatusEnum, RuletextEnum } from "@/utils/enum";
  ...
  computed: {
    ...
    daylist() {
      ...
      if (this.val.length === howDay.currentDay) {
        //天天送
        this.val.map((el: any) => {
          const id = el.deliveryDate.split("-")[2];
          el.id = +id;
          return el;
        });
        return this.val;
