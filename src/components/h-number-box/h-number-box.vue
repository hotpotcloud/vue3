<template>
  <view class="u-number-box">
    ...
    >
      <!-- <slot name="minus" /> -->
      <image
        class="minus-btn marR16rpx"
        :src="require('@/assets/jian.png')"
        mode="aspectFit"
      />
    </view>
    <view
 ...
 * @property {String | Number}	inputHeight		输入框宽度，单位为px （默认30 ）
 * @property {String | Number}	inputRadius		输入框宽度，单位为px （ 默认8）
 ...
 *@property {Object}			CSS-style		输入框样式
 *
  ...
  computed: {
    ...
    inputStyle() {
      ...
      const style = {
        color: this.color,
        backgroundColor: this.bgColor,
        height: uni.$u.addUnit(this.inputHeight || this.buttonSize),
        width: uni.$u.addUnit(this.inputWidth),
        // "border-radius": "16rpx",
        // margin: "0 -2rpx 0 16rpx",
        borderRadius: uni.$u.addUnit(this.inputRadius),
      };
      return style;
    },
    // 用于监听多个值发生变化
