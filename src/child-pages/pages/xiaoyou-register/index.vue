...
export default {
  ...
  onLoad(option) {
    ...
    if (option.scene) {
      const [shareKey, shareId] = decodeURIComponent(option.scene).split("&");
      console.log("shareId", shareKey, shareId);
      if (shareKey === "distribution") {
        //扫码进入
        console.log("1--option", 1);
        this.pid = shareId;
        uni.setStorageSync("pid", shareId);
        this.v_setDistId(shareId);
      } else {
        //分享进入
